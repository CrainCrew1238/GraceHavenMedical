...
type Dict = any; // tighten if you have types
type Props = { dict: Dict };

export default function ContactUsClient({ dict }: Props) {
    return (
        <div>
            <Script
                id="vagaro-script"
                ...
                strategy="lazyOnload"
                // If widgetJS is server-only (not NEXT_PUBLIC_), this still works because it
                // renders as inline text on the server into the HTML.
                dangerouslySetInnerHTML={{ __html: process.env.widgetJS || '' }}
            />
